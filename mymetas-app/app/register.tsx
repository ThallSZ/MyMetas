...
const RegisterScreen = () => {
  ...
  const handleRegister = async () => {
    ...
    setLoading(true);
    const { error } = await supabase.auth.signUp({ email: email.trim(), password: password.trim(), options: { data: { full_name: name.trim() } } });
    setLoading(false);
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={styles.innerContainer}>
        <FontAwesome5 name="user-plus" size={50} color={colors.primary} style={{ alignSelf: 'center', marginBottom: 20 }} />
        <Text style={[styles.title, { color: colors.text }]}>Criar Conta</Text>
        <Text style={[styles.subtitle, { color: colors.textSecondary }]}>Comece a organizar suas metas hoje.</Text>
        
        <AuthInput icon="user" placeholder="Nome Completo" value={name} onChangeText={setName} />
        ...
        {loading ? (
          ...
          <>
            ...
            <TouchableOpacity style={styles.linkButton} onPress={() => router.back()}>
              <Text style={[styles.linkButtonText, { color: colors.primary }]}>Já tenho uma conta.</Text>
            </TouchableOpacity>
          </>
        )}
      </View>
