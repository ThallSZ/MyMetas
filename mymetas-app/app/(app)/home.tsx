...
const Countdown = ({ targetDate, status }: { targetDate: string | null, status: string }) => {
  ...
  if (status === 'a_fazer') {
    return <Text style={[styles.countdownToDo, { color: colors.textSecondary }]}>A fazer</Text>;
  }
  ...
  return <Text style={[styles.countdown, { color: colors.primary }]}>{daysLeft} dias</Text>;
};
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <Modal animationType="slide" transparent={true} visible={modalVisible} onRequestClose={() => setModalVisible(false)}>
        <View style={styles.modalContainer}>
          <View style={[styles.modalView, { backgroundColor: colors.card }]}>
            ...
            <TextInput style={[styles.input, { backgroundColor: colors.inputBackground, color: colors.text, borderColor: colors.border }]} placeholderTextColor={colors.placeholder} placeholder="Título da sua meta" value={newMetaTitle} onChangeText={setNewMetaTitle} />
            <TextInput style={[styles.input, { backgroundColor: colors.inputBackground, color: colors.text, borderColor: colors.border }]} placeholderTextColor={colors.placeholder} placeholder="Descrição (opcional)" value={newMetaDescription} onChangeText={setNewMetaDescription} />
            ...
            <View style={styles.modalButtons}>
              <TouchableOpacity style={[styles.button, { backgroundColor: colors.cancelButton }]} onPress={() => setModalVisible(false)}><Text style={[styles.buttonText, { color: colors.cancelButtonText }]}>Cancelar</Text></TouchableOpacity>
              <TouchableOpacity style={[styles.button, { backgroundColor: colors.primary }]} onPress={handleCreateMeta}><Text style={styles.buttonText}>Salvar</Text></TouchableOpacity>
            </View>
          </View>
        </View>
      </Modal>
      ...
      <View style={styles.header}>
        <Text style={[styles.title, { color: colors.text }]}>Minhas Metas</Text>
      ...
      <View style={[styles.searchContainer, { backgroundColor: colors.card, shadowColor: colors.text }]}>
        ...
        <TextInput style={[styles.inputSearch, { color: colors.text }]} placeholderTextColor={colors.placeholder} placeholder="Buscar por título..." value={searchTerm} onChangeText={setSearchTerm} onSubmitEditing={fetchMetas} />
      </View>
      ...
      {loading && metas.length === 0 ? <ActivityIndicator size="large" style={{marginTop: 50}}/> : (
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity style={[styles.metaItem, { backgroundColor: colors.card, shadowColor: colors.text }]} onPress={() => router.push(`/meta/${item.id}`)}>
              <View style={styles.metaInfo}>
                <Text style={[styles.metaTitle, { color: colors.text }]}>{item.favorite ? "★ " : ""}{item.title}</Text>
              </View>
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
                ...
                <Text style={[styles.emptyText, { color: colors.textSecondary }]}>Nenhuma meta encontrada.</Text>
                <Text style={[styles.emptySubText, { color: colors.border }]}>Clique no botão "+" para começar.</Text>
            </View>
          }
...
const styles = StyleSheet.create({
  ...
  headerAvatarPlaceholder: { width: 44, height: 44, borderRadius: 22, justifyContent: 'center', alignItems: 'center' },
  title: { fontSize: 32, fontWeight: 'bold' },
  searchContainer: { flexDirection: 'row', alignItems: 'center', borderRadius: 8, marginHorizontal: 16, marginBottom: 16, elevation: 2, shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 2 },
  searchIcon: { padding: 12 },
  inputSearch: { flex: 1, height: 48, fontSize: 16 },
  metaItem: { padding: 20, borderRadius: 12, marginHorizontal: 16, marginBottom: 12, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', elevation: 2, shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 2 },
  metaInfo: { flex: 1, marginRight: 8 },
  metaTitle: { fontSize: 18, fontWeight: '600' },
  countdown: { fontSize: 14, fontWeight: 'bold' },
  ...
  countdownToDo: { fontSize: 14, fontWeight: 'bold' },
  ...
  fab: { position: 'absolute', right: 20, bottom: 80, width: 60, height: 60, borderRadius: 30, justifyContent: 'center', alignItems: 'center', elevation: 5, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.2, shadowRadius: 3 },
