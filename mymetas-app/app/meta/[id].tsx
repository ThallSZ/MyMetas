...
import { View, Text, StyleSheet, Alert, FlatList, ActivityIndicator, TouchableOpacity, TextInput, Button, Modal, Platform, ScrollView } from 'react-native';
...
const Countdown = ({ targetDate, status, completedDate }: { targetDate: string | null, status: string, completedDate: string | null }) => {
  if (status === 'concluida') {
    return <Text style={styles.countdownDone}>Concluída em {format(new Date(completedDate || Date.now()), 'dd/MM/yy')}</Text>;
  }
  ...
  if (isPast(target) && !isToday(target)) {
    return <Text style={styles.countdownOverdue}>Atrasado em {Math.abs(daysLeft)} dias</Text>;
  }
  if (daysLeft === 0) {
    return <Text style={styles.countdownToday}>Termina hoje!</Text>;
  }
  return <Text style={styles.countdown}>Faltam {daysLeft} dias</Text>;
};
...
const MetaDetailsScreen = () => {
  ...
  if (!meta) { return <View style={styles.centered}><Text>Meta não encontrada.</Text></View>; }
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <Modal animationType="slide" transparent={true} visible={editModalVisible} onRequestClose={() => setEditModalVisible(false)}>
        <View style={styles.modalContainer}>
          <View style={styles.modalView}>
            ...
            <TextInput style={styles.input} placeholder="Título" value={editingMeta.title} onChangeText={(text) => setEditingMeta({...editingMeta, title: text})} />
            <TextInput style={styles.input} placeholder="Descrição" value={editingMeta.description} onChangeText={(text) => setEditingMeta({...editingMeta, description: text})} multiline/>
            
            <Text style={styles.label}>Status</Text>
            <View style={styles.pickerContainer}>
              <Picker selectedValue={editingMeta.status} onValueChange={(itemValue) => setEditingMeta({...editingMeta, status: itemValue})} style={styles.picker}>
            ...
            <TouchableOpacity onPress={() => setShowPicker(true)}>
                <TextInput style={styles.input} value={format(date, 'dd/MM/yyyy')} editable={false} />
            </TouchableOpacity>
            ...
            <View style={styles.modalButtons}>
              ...
              <TouchableOpacity style={styles.button} onPress={handleUpdateMeta}><Text style={styles.buttonText}>Salvar</Text></TouchableOpacity>
            </View>
          </View>
        </View>
      </Modal>
      ...
      <View style={styles.card}>
        ...
        <Text style={styles.description}>{meta.description || 'Sem descrição.'}</Text>
        <View style={styles.statusContainer}>
            <Text style={styles.statusLabel}>Status:</Text>
            <Text style={styles.statusValue}>{formatStatus(meta.status)}</Text>
        </View>
        {meta.date_target && <Text style={styles.dateLabel}>Prazo: {format(new Date(meta.date_target), 'dd MMMM, yyyy', { locale: ptBR })}</Text>}
      ...
      <View style={styles.card}>
        ...
        <View style={styles.addStepContainer}>
          <TextInput style={styles.inputStep} placeholder="Adicionar nova etapa..." value={newStepDescription} onChangeText={setNewStepDescription} />
          <TouchableOpacity style={styles.addStepButton} onPress={handleCreateStep}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View style={styles.stepItem}>
              <TouchableOpacity onPress={() => handleToggleStep(item)} style={styles.stepToggle}>
                <FontAwesome name={item.done ? "check-square-o" : "square-o"} size={24} color={item.done ? "#2ecc71" : "#bdc3c7"} />
                <Text style={item.done ? styles.stepDone : styles.stepText}>{item.description}</Text>
              </TouchableOpacity>
          ...
          ListEmptyComponent={<Text style={styles.emptyText}>Nenhuma etapa cadastrada.</Text>}
        />
      </View>
    </ScrollView>
  );
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: '#fff', borderRadius: 12, padding: 20, marginBottom: 16, elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 2 },
  cardTitle: { fontSize: 22, fontWeight: 'bold', marginBottom: 12, color: '#333' },
  ...
  statusLabel: { fontSize: 16, color: 'gray', marginRight: 8 },
  statusValue: { fontSize: 16, fontWeight: '600', color: '#333' },
  dateLabel: { fontSize: 16, color: 'gray', marginBottom: 16 },
  actionsContainer: { flexDirection: 'row', justifyContent: 'space-around', backgroundColor: '#fff', borderRadius: 12, paddingVertical: 10, marginBottom: 16, elevation: 2 },
  actionButton: { alignItems: 'center' },
  actionButtonText: { marginTop: 4, color: '#555' },
  addStepContainer: { flexDirection: 'row', marginBottom: 16, alignItems: 'center' },
  inputStep: { flex: 1, height: 44, borderColor: '#eee', borderWidth: 1, borderRadius: 8, paddingHorizontal: 10, backgroundColor: '#f9f9f9' },
  addStepButton: { width: 44, height: 44, borderRadius: 22, backgroundColor: '#007BFF', justifyContent: 'center', alignItems: 'center', marginLeft: 8 },
  stepItem: { paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#f0f0f0', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  stepToggle: { flexDirection: 'row', alignItems: 'center', flex: 1 },
  stepText: { fontSize: 16, marginLeft: 12, color: '#333' },
  stepDone: { fontSize: 16, marginLeft: 12, textDecorationLine: 'line-through', color: 'gray' },
  emptyText: { color: 'gray', textAlign: 'center', paddingVertical: 20 },
  countdown: { fontSize: 16, color: '#007BFF', fontWeight: 'bold', marginTop: 8, textAlign: 'center' },
  ...
  countdownDone: { fontSize: 16, color: '#2ecc71', fontWeight: 'bold', marginTop: 8, textAlign: 'center' },
  ...
  pickerContainer: { width: '100%', borderColor: '#ddd', borderWidth: 1, borderRadius: 8, marginBottom: 15, justifyContent: 'center' },
  picker: { width: '100%' },
